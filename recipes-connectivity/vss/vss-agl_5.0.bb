...
SRC_URI += "file://agl_vss_overlay.vspec \
            ...
            file://agl_vss_overlay.vspec.gw-single \
...
do_compile() {
    ...
    vspec export json ${VSPEC_JSON_OPTS} -l ${WORKDIR}/agl_vss_overlay.vspec.gw-single --output vss_${PV}-agl-gw-single.json
...
do_install() {
    ...
    install -m 0644 vss_${PV}-agl-gw-single.json ${D}${datadir}/vss/
    install -m 0644 vss_${PV}-agl-gw-control-panel.json ${D}${datadir}/vss/
    install -m 0644 vss_${PV}-agl-gw-hardware.json ${D}${datadir}/vss/
}

PACKAGE_BEFORE_PN += "${PN}-control-panel ${PN}-control-panel-all ${PN}-gw-single ${PN}-gw-control-panel ${PN}-gw-hardware"
...
ALTERNATIVE:${PN}-gw-single = "vss.json"
ALTERNATIVE_TARGET_${PN}-gw-single = "${datadir}/vss/vss_${PV}-agl-gw-single.json"
ALTERNATIVE_PRIORITY_${PN}-gw-single = "32"
FILES:${PN}-gw-single += "${datadir}/vss/vss_${PV}-agl-gw-single.json"
